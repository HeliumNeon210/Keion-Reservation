  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 gap-4">
        ...
        <p className="text-slate-500 font-bold animate-pulse tracking-widest text-sm">データを同期中...</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-10">
        ...
        <div className="flex items-center gap-2">
          <button 
            ...
            className={`px-3 py-1.5 rounded-full text-[10px] md:text-xs font-black transition-all shadow-sm ${
              role === 'ADVISOR' ? 'bg-amber-400 text-slate-900 scale-105' : 'bg-indigo-600 border border-indigo-500 hover:bg-indigo-50'
            }`}
          >
            {role === 'ADVISOR' ? '顧問' : '生徒'}
          </button>
      ...
      <main className="max-w-5xl mx-auto p-4 md:p-8 space-y-6">
        <div className="flex items-center justify-between bg-white p-5 rounded-2xl shadow-sm border border-slate-200">
          <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1))} className="p-2 hover:bg-slate-100 rounded-xl transition-colors"><ChevronLeftIcon className="w-6 h-6" /></button>
          <h2 className="text-xl font-black text-slate-800 tracking-tighter">{format(currentMonth, 'yyyy年 MMMM', { locale: ja })}</h2>
          <button onClick={() => setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1, 1))} className="p-2 hover:bg-slate-100 rounded-xl transition-colors"><ChevronRightIcon className="w-6 h-6" /></button>
        </div>
        ...
        <div className="grid grid-cols-7 gap-1 md:gap-2">
          ...
          {Array.from({ length: getDay(new Date(currentMonth.getFullYear(), currentMonth.getMonth(), 1)) }).map((_, i) => <div key={`empty-${i}`} className="bg-slate-200/20 rounded-xl h-32 md:h-40" />)}
          {days.map(date => {
            const dateStr = format(date, 'yyyy-MM-dd');
                ...
                <div className="space-y-1">
                  {dateBookings.sort((a,b) => a.timeSlot.localeCompare(b.timeSlot)).map(b => (
                    <div key={b.id} className="text-[10px] p-1.5 bg-indigo-50/50 text-indigo-700 rounded-lg truncate border border-indigo-100 font-bold">
                      <span className="opacity-50 mr-1 text-[8px]">{b.timeSlot.split('-')[0]}</span>{b.bandName}
                    </div>
                  ))}
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden">
            <div className="bg-indigo-700 p-8 text-white relative">
              ...
              <button onClick={() => { setIsBookingModalOpen(false); setConfirmingDeleteId(null); }} className="absolute right-6 top-8 p-2 hover:bg-white/10 rounded-full transition-colors">✕</button>
            </div>
            ...
            <div className="p-8 space-y-5 max-h-[60vh] overflow-y-auto">
                    ...
                    <div key={slot} className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all ${booking ? 'border-indigo-100 bg-indigo-50/20' : 'border-slate-100 bg-white hover:border-indigo-100'}`}>
                      ...
                      {booking ? (
                        ...
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => {
                            const val = (document.getElementById(`band-${slot}`) as HTMLInputElement).value;
                            handleAddBooking(dateStr, slot, val);
                            (document.getElementById(`band-${slot}`) as HTMLInputElement).value = "";
                          }} className="bg-indigo-600 text-white p-3 rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-100 active:scale-95 transition-all">
              ...
              {confirmingDeleteId && (
                <div className="p-4 bg-red-50 border border-red-200 rounded-2xl mt-4 animate-pulse">
                  <p className="text-xs font-black text-red-700 mb-3 text-center">本当に予約を削除しますか？</p>
                  <div className="flex gap-2">
                    <button onClick={() => executeDelete(confirmingDeleteId)} className="flex-1 bg-red-600 text-white py-2 rounded-xl text-xs font-black">削除実行</button>
                    <button onClick={() => setConfirmingDeleteId(null)} className="flex-1 bg-white border border-slate-200 text-slate-500 py-2 rounded-xl text-xs font-black">キャンセル</button>
                  </div>
                </div>
              )}
            </div>
            <div className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end">
              <button onClick={() => { setIsBookingModalOpen(false); setConfirmingDeleteId(null); }} className="px-8 py-3 bg-white border border-slate-200 text-slate-600 rounded-2xl font-black shadow-sm hover:bg-slate-100 transition-all">閉じる</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            <div className="bg-amber-500 p-8 text-white shrink-0">
              <h3 className="text-2xl font-black tracking-tight">週間スケジュールの管理</h3>
              <p className="text-amber-100 text-sm font-bold uppercase tracking-widest">Weekly Settings</p>
            </div>
